...
const T = {
  ...
  expired: { kk: 'Төленбеген', ru: 'Не оплачено' },
  expiredHint: { kk: 'сайт жабық', ru: 'сайт закрыт' },
...
export default async function SubscriptionsPage() {
  ...
  const now = Date.now();
  const expired = subs.filter((s) => periodIsOver(s.periodEnd, now));
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label={T.active[locale]} value={subs.length - expired.length} />
        <StatCard label={T.expired[locale]} value={expired.length} hint={T.expiredHint[locale]} />
