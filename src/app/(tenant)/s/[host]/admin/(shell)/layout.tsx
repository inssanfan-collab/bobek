...
const T = {
  ...
  suspendedHint: {
    kk: 'Жазылым төленбеген, сондықтан сайт келушілер үшін жабық. Әкімші бөлімі тек қарау режимінде. Төлем түскен соң сайт бірден ашылады — портал әкімшісіне хабарласыңыз.',
    ru: 'Подписка не оплачена, поэтому сайт закрыт для посетителей. Админка — только для просмотра. После оплаты сайт откроется сразу — свяжитесь с администратором портала.',
  },
  blockedHint: {
...
export default async function TenantAdminLayout({
  ...
  const nav: NavItem[] = [
    ...
    { href: `${base}/homepage`, label: n.homepage[locale], icon: '🏠' },
    ...
    <>
      ...
      {ctx.suspended ? (
        <div className="px-4 pt-4 sm:px-6">
          <Alert tone="danger" title={T.suspended[locale]}>
            {T.suspendedHint[locale]}
          </Alert>
        </div>
