...
export async function buildContractPdf(data: DocData): Promise<Buffer> {
  ...
  b.columns({
    kk: '7.3. Төлем болмаған жағдайда сайт 30 күн бойы қолжетімді болып қалады, әкімшілік панель тек оқу режиміне ауысады. Осы мерзім өткен соң қол жеткізу тоқтатылады, деректер тағы 90 күн сақталады және Тапсырыс берушінің сұрауы бойынша беріледі.',
    ru: '7.3. При отсутствии оплаты сайт остаётся доступным 30 дней, административная панель переводится в режим только для чтения. По истечении этого срока доступ приостанавливается, данные хранятся ещё 90 дней и передаются Заказчику по его запросу.',
  });
  b.columns({
