...
export async function buildContractPdf(data: DocData): Promise<Buffer> {
  ...
  b.columns({
    kk: '1.4. edu.kz аймағындағы жеке доменді тіркеу және оның ақысы қызметке кірмейді: оны Тапсырыс беруші өз атына сатып алады, Орындаушы баптауға өтеусіз көмектеседі.',
    ru: '1.4. Регистрация и оплата собственного домена в зоне edu.kz в состав услуги не входят: Заказчик приобретает его на своё имя, Исполнитель безвозмездно помогает с настройкой.',
  });
  ...
  b.columns({
    kk: '7.3. Төлем болмаған жағдайда сайт 30 күн бойы жұмыс істейді, әкімшілік панель тек қарау режиміне ауысады. Осы мерзім өткен соң қызмет тоқтатылады: сайт келушілер үшін жабылады, оның орнына «Сайт уақытша қолжетімсіз» беті мен Тапсырыс берушінің телефоны көрсетіледі. Төлем түскен соң сайт бұрынғы мазмұнымен бірден ашылады. Деректер тоқтатылғаннан кейін 90 күн сақталады және Тапсырыс берушінің сұрауы бойынша беріледі.',
    ru: '7.3. При отсутствии оплаты сайт работает ещё 30 дней, административная панель переводится в режим только для просмотра. По истечении этого срока услуга приостанавливается: сайт закрывается для посетителей, вместо него показывается страница «Сайт временно недоступен» с телефоном Заказчика. После поступления оплаты сайт открывается снова, со всем прежним содержимым. Данные хранятся 90 дней после приостановления и передаются Заказчику по его запросу.',
  });
  b.columns({
