...
# Ключ геокодера Яндекса — по адресу сада определяются координаты для карты
# в каталоге. Берётся в кабинете разработчика Яндекса, тариф «HTTP Геокодер».
# Ключ серверный, в браузер не попадает. Без него координаты вводятся вручную
# в паспорте сада — платформа работает, карта просто пустеет.
YANDEX_GEOCODER_KEY="<ключ из кабинета Яндекса>"

# Ключ JavaScript API — интерактивная карта в каталоге. Этот уезжает в браузер.
# Бесплатный тариф — 100 запросов в сутки; без ключа каталог показывает виджет
# Яндекса в iframe, тому ключ не нужен и лимита у него нет.
YANDEX_MAPS_KEY="<ключ из кабинета Яндекса>"
...
На боевом сервере копируется **только база** — каждую ночь в 03:30,
таймером `vsesad-backup.timer`. Загруженные файлы садов не копируются
(решение владельца от 25.09.2026: места под них нет, после загрузки файлы
не меняются). Установка — один раз, из свежего кода:

```bash
cd /var/www/vsesad
install -m 755 deploy/vsesad-backup deploy/vsesad-restore /usr/local/bin/
cp deploy/vsesad-backup.service deploy/vsesad-backup.timer /etc/systemd/system/
systemctl daemon-reload && systemctl enable --now vsesad-backup.timer
/usr/local/bin/vsesad-backup          # первая копия — сразу
```

Копии — в `/var/backups/vsesad` (только root), хранятся 30 дней. Каждая
проверяется чтением оглавления; не получилось — письмо на `NOTIFY_EMAIL`.
Состояние видно в админке портала, раздел «Сервер».

Восстановление: `vsesad-restore` без аргументов покажет список копий,
`vsesad-restore <файл>` заменит базу, предварительно сняв копию текущего
состояния. **Раз в месяц проверяйте восстановление** на отдельной базе —
непроверенная копия копией не является (команда — в разделе «Сервер»).

Копии лежат на том же сервере, что и данные: от ошибок они спасают, от потери
сервера — нет. Для этого нужна выгрузка в другое место в Казахстане.
...
Сады, заведённые до появления геокодера, стоят на карте без координат. Добрать их:

```bash
docker compose exec app pnpm maps:geocode        # только те, у кого координат нет
docker compose exec app pnpm maps:geocode --all  # пересчитать все
```

`--all` затирает и то, что заведующая ставила руками, — без него не обходятся
только при переносе садов со старой базы.

## 8. Тесты

```bash
pnpm test          # юнит-тесты: резолв доменов, языки, санитайзер, адреса, поиск
